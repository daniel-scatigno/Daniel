...
namespace Daniel.Blazor.Components
{
   public partial class Listing<TViewModel> where TViewModel : class, new()
   {
      ...
      public async Task Teste()
      {
         ...
         await Task.Yield();
         AppManager.ShowNotification("Registro salvo com sucesso!", null, Daniel.Blazor.Services.ToastType.Info);
      }
      ...
      private async Task DeleteRecord()
      {
         ...
         if (SelectedLine != null)
         {
            if (DataSource == null)
            {
               ...
               //TODO Localizar
               AppManager.ShowNotification("Registro removido com sucesso", null, Services.ToastType.Info);
            }
            else
            {
